...
export default function Login(){
    ...
    function formhandle(e){
        e.preventDefault()
        fetch('http://localhost:3001/tlogincheck',{
            method:'POST',
            headers:{
    ...
    return(
      ...
      <div className={styles.loginContainer}>
        <h1 style={{"color":"black"}}>Teachers Login</h1>
        <form onSubmit={formhandle}>
          <div className={styles.inputFieldContainer}>
            <label className={styles.label} htmlFor="username">Phone</label>
            <input type="text" id="username" className={styles.inputField}  onChange={(e)=>setPhone(e.target.value)} placeholder="Enter your Phone" required />
          </div>
          <div className={styles.inputFieldContainer}>
        ...
        <p className={styles.registerText}>Don't have an account? <Link to="/newusert" className={styles.registerLink}>Register Here</Link></p>
      </div>
